...
export async function createRandomSale(payload) {
  const res = await fetch("http://127.0.0.1:8000/sales/manual/random", {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    throw new Error("Failed to create random sale");
  }
